package com.madcrow.gun_class.CustomExceptions;
...
@ControllerAdvice //to share across all controllers
@RestController
public class CustomResponseHandler extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<Object> handleAllException(PostNotFoundException ex, WebRequest request)
    {
        ExceptionResponse exceptionResponse =  new ExceptionResponse(new Date(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(exceptionResponse, HttpStatus.NOT_FOUND);
    }
